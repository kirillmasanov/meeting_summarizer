...
|--------|------------|-------------------------------------------|
| **Yandex SpeechKit API v3** | Асинхронное распознавание речи из аудиофайлов с поддержкой длинных записей (до 1 часа) и встроенной суммаризацией через AI Studio Model Gallery | - `ai.speechkit-stt.user` — доступ к API распознавания речи<br>- `ai.languageModels.user` — доступ к языковым моделям для суммаризации |
